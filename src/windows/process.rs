...
use winapi::tlhelp32::{TH32CS_SNAPTHREAD, THREADENTRY32};
...
impl Process {
    ...
    pub fn new(handle: HANDLE, pid: u32, start_time: u64) -> Process {
        ...
        Process {
            ...
            pid: pid,
            cmd: String::new(),
            environ: unsafe { get_proc_env(handle, pid) },
            exe: String::new(),
            cwd: String::new(),
            root: String::new(),
            memory: 0,
...
unsafe fn get_proc_env(handle: HANDLE, pid: u32) -> Vec<String> {
    let mut ret = Vec::new();
        ...
        if kernel32::Thread32First(snapshot_handle, &mut target_thread) == TRUE {
            ...
            loop {
                if target_thread.th32OwnerProcessID == pid {
                    println!("4");
                    let thread_handle = kernel32::OpenThread(THREAD_SUSPEND_RESUME | THREAD_QUERY_INFORMATION,
                                                             FALSE,
                        ...
                        if kernel32::SuspendThread(thread_handle) != DWORD::max_value() {
                            ...
                            if kernel32::GetThreadContext(thread_handle, &mut context) != 0 {
                                ...
                                if kernel32::ReadProcessMemory(handle,
                                                               context.SegFs as usize as *mut winapi::c_void,
